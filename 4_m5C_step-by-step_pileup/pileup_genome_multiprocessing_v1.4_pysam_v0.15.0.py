...
def worker(conting,start,stop):
	...
	if "GL" in conting:
		...
		max_depth = options.max_depth
	with pysam.AlignmentFile(options.input, 'rb') as samfile, file(tmp_file_name,'a') as tmp_file:
		try:
			for pileupcolumn in samfile.pileup(conting,start=start,stop=stop,max_depth=max_depth,ignore_orphans=False,ignore_overlaps=False,truncate=True):
